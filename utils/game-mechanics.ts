...
const handleNextLevel = () => {
  useGameStore.setState((state) => ({
    ...state,
    gameLevelIndex: state.gameLevelIndex + 1, // Увеличиваем индекс уровня
  }));
};

// Функция для перехода на предыдущий уровень
const handlePrevLevel = () => {
  useGameStore.setState((state) => ({
    ...state,
    gameLevelIndex: state.gameLevelIndex - 1, // Уменьшаем индекс уровня
  }));
};